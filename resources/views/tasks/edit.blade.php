<x-app-layout>
    <div class="shadow bg-white border rounded w-full max-w-md mx-auto mt-10">
        ...
        <form action="{{ route('tasks.update', $task) }}" method="POST" class="relative px-6 pb-6 flex-auto">
            ...
            <div class="my-4 text-slate-500 text-lg leading-relaxed mb-2">
                ...
                <textarea name="body" id="body"
                    placeholder="メモ"class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline h-32">{{ old('body', $task->body) }}</textarea>
            </div>
        ...
        <form action="{{ route('tasks.destroy', $task) }}" method="POST" class="relative px-6 pb-6 flex-auto">
            @csrf
            @method('DELETE')
